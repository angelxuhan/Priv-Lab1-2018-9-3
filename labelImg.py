...
###################这部分代码开始加入整个布局了#####################
    ...
    def load_point(self,filepath):
        ...
        x = 0
        y = 0
        print("self.canvas.point_shape",self.canvas.point_shape)
        if os.path.exists(filepath):
            with open(filepath) as infile:
                lines = infile.readlines()
    ...
    def openNextImg(self, _value=False):
        ...
        if self.autoSaving is True and self.defaultSaveDir is not None and not self.image.isNull():
            if self.dirty is True or self.task_mode ==3 or self.task_mode==4 :#模式4下的这里的下一张图片的自动保存有问题
                self.saveFile()
        if self.task_mode==4:
            self.point_next_img()
